...
namespace Graph {
    ...
    public:
        ...
        std::vector<EdgeId> AddEdges(std::vector<Edge<Weight>> edges);
    ...
    template <typename Weight>
    std::vector<EdgeId> DirectedWeightedGraph<Weight>::AddEdges(std::vector<Edge<Weight>> edges)
    {
        std::vector<EdgeId> ids(edges.size());
        for (size_t i = 0; i < edges.size(); ++i)
        {
            ids[i] = AddEdge(edges[i]);
        }
        return ids;
    }
